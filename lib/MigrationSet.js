 ...
 *
 * @param {Object} db
 * @param {Object} migrationStorageController
 * @param {function} complete
 * @api private
 */
function Set(migrationScriptResources, migrationStorageController, logger, complete) {
	this.migrationScriptResources = migrationScriptResources;
	this.migrations = [];
	this.pos = 0;
	this.migrationStorageController = migrationStorageController;
 ...
 */
_.extend(Set.prototype, EventEmitter.prototype);
...
Set.prototype._migrate = function (direction, fn, lastMigrationNum) {
	...
	function next(err, migration) {
		...
		try {
			...
			migration[direction](self.migrationScriptResources, function (migrationErr) { // Call Migration function
				if (migrationErr) {
					console.error('Error inside migration: ', migration.title, '\nError: ', migrationErr);
					//Revert this migration the opposite way
					return migration[direction === 'up' ? 'down' : 'up'](self.migrationScriptResources, function (migrateDownErr) {
						var msg = '';
                        if (migrateDownErr) {
                            msg = 'Error migrating back down: ' + migration.title + '\nerr: ' + migrateDownErr + '\nThe database may be in a corrupted state!';
                ...
                if (nextCalled) {
                    // The complete function should only be called once per migration but async code can sometimes allow a migration to call it twice, error when this happens
                    throw new Error('The next method should not be called more than once from the same migration script.');
                }

                nextCalled = true;

				if (isDirectionUp) {
                    self.migrationStorageController.migrationEntryExists(migration, function (err, exists) {
                        if (err) throw new Error(err);
                        if (exists) throw new Error('Attempt to create duplicate migration entries in the migration storage container (database)');

                        self.migrationStorageController.addMigrationEntry(migration, function(err, objects){
                            if (err) {
                                return self.complete('Error saving migration run: ' + migration.title + '\nerr: ' + err);
                    ...
                    // TODO (CAW): Rewrite down migration to drive from db collection rather than file list
                    self.migrationStorageController.removeMigrationEntry(migration, function (err, doc) {
                        if (err) {
                            // log error but continue as this can happen when there are new migration scripts
                            self.logger.log('Error', 'removing migration from DB: ' + migration.title, true);
			...
			console.error('Error inside migration: ', migration.title, '\nError: ', ex);
			migration[direction === 'up' ? 'down' : 'up'](self.migrationScriptResources, function (migrateDownErr) {
				var msg = '';
                if (migrateDownErr) {
					msg = 'Error migrating back down: ' + migration.title + '\nerr: ' + migrateDownErr + '\nThe database may be in a corrupted state!';
					// console.error('Error migrating back down: ', migration.title, '\nerr: ', migrateDownErr);
